...
module lsa_row(n=3) {
  ...
  $width_difference = 5;
  $height_difference = 5;
  $dish_type = "disable"; // disable, spherical
  $dish_depth = 0.85;
  $dish_skew_x = 0;
  $dish_skew_y = 0;
  $top_skew = 0;
...
module lego_row(n=3) {
    ...
    $height_slices = 10;
    $corner_radius = 0.25;  
    $total_depth = unitMbu * baseLayers * 2 - 0.1;  // H pin
    $top_tilt = 0;
   ...
   union() {
        // Lego Brick
        translate([-brick_p * brickSizeX /2, -brick_p * brickSizeX /2, 0])
         brick ();
...
module c64_row(n=1) {
    // stem("cherry", 5, $stem_slop);
    //stem("rounded_cherry", 5, $stem_slop);
    //stem("box_cherry", 5, $stem_slop);
    ...
    union() {
        ...
        $C64H = 3.5;
        translate([0,0,6]) cube([4.5,1.4, $C64H], true);
        translate([0,0,6]) cube([1.4,4.5, $C64H], true);    
    }
}

module dsa_flat_row(n=3) {
...
module key_profile(key_profile_type, row) {
  ...
  } else if (key_profile_type == "lsa") {
    lsa_row(row) children();  
  } else if (key_profile_type == "sa") {
    sa_row(row) children();
  } else if (key_profile_type == "g20") {
    g20_row(row) children();
  } else if (key_profile_type == "lego") {
    lego_row(row) children();
  } else if (key_profile_type == "c64") {
