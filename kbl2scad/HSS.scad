...
module pin_wire_slots(){
    translate([pin1.x, 3*grid, -base.z/2])
     cube([1.2*wire_dia, 2.5*grid, 3*wire_dia], center = true);

    translate([pin2.x, 3*grid, -base.z/2])
     cube([1.2*wire_dia, 2.5*grid, 3*wire_dia], center = true);

    translate([pin2.x, pin2.y - 3, 0])
     cylinder(h=base.z*2, d=pin2[2], center = true);
    
    translate([pin1.x, pin1.y + 3, 0])
...
module make_body(side_slot = true){
    difference(){
        translate([0, 0, 0]){
